...
export default function OperacionPage() {
  ...
  // ── Estado: Búsqueda y Finalizar Cremación ──────────────────────────────
  const [query, setQuery] = useState("");
  const [finalizandoCremacion, setFinalizandoCremacion] = useState<string | null>(null);
  ...
  const finalizarCremacion = async (sesionId: string) => {
    setFinalizandoCremacion(sesionId);
    try {
      const res = await fetch(`/api/sesiones-cremacion/${sesionId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const filteredOrders = q
    ? orders.filter(o =>
        ...
        o.pet?.name?.toLowerCase().includes(q) ||
        o.owner?.name?.toLowerCase().includes(q)
      )
    : orders;

  const stageCounts = STEP_LABELS.reduce<Record<string, number>>((acc, s) => {
              ...
              {STEP_LABELS.map((s, i) => {
                ...
                return (
                  <div key={s} className="flex items-center gap-2 flex-shrink-0">
                    <div className={`flex items-center gap-1.5 text-[10px] font-black uppercase px-3 py-1.5 rounded-full border ${info.color}`}>
                      {cnt > 0 && <span className="bg-black/30 px-1.5 py-0.5 rounded-full text-[9px]">{cnt}</span>}
                      {info.label}
                    </div>
                    {i < STEP_LABELS.length - 1 && <ChevronRight size={12} className="text-slate-700 flex-shrink-0" />}
                  </div>
                );
              })}
            </div>
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="Buscar por folio, mascota o dueño..."
                  className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 pl-9 pr-4 text-xs text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-brand-gold-500/40 transition-all"
                />
                {query && (
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-slate-400 text-xs font-black uppercase tracking-widest transition-all flex-shrink-0"
              >
                <RefreshCcw size={13} className={loadingOrders ? "animate-spin" : ""} />
              </button>
            </div>
          </div>
          ...
          {loadingOrders ? (
            ...
            <div className="glass-card rounded-[2rem] p-16 text-center">
              ...
              <h3 className="text-xl font-bold text-slate-400">
                {query ? `Sin resultados para "${query}"` : "¡Sin pendientes!"}
              </h3>
              <p className="text-slate-600 text-sm mt-2">
                {query ? "Intenta otra búsqueda." : "No hay servicios activos en este momento."}
              </p>
            </div>
          ) : (
            <AnimatePresence>
                    ...
                    >
                      ...
                      <div className="p-6 flex flex-col gap-4 flex-1">
                        ...
                        <div className="flex flex-col gap-2 mt-1">
                          ...
                          {order.status === "CREMATING" && order.sesionCremacion && (
                            <button
                              onClick={() => finalizarCremacion(order.sesionCremacion!.id)}
                              disabled={finalizandoCremacion === order.sesionCremacion!.id}
                              className="w-full py-2.5 rounded-2xl flex items-center justify-center gap-2 font-bold text-sm bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 hover:bg-emerald-500/20 transition-colors disabled:opacity-50"
                            >
                              {finalizandoCremacion === order.sesionCremacion!.id
                                ? <><Loader2 size={15} className="animate-spin" /> Finalizando...</>
                                : <><CheckCircle2 size={15} /> Finalizar Cremación</>}
                            </button>
                          )}
