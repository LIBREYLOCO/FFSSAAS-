...
interface Bokeh {
  x: number; y: number; r: number;
  opacity: number; vx: number; vy: number; color: string;
}
...
export default function BackgroundLayer({ backgroundId }: Props) {
  const bg = getBackground(backgroundId);

  return (
    <>
      {/* Photo background — rendered only when a photo is selected */}
      {bg.url && (
        <div
          aria-hidden="true"
          className="fixed inset-0 pointer-events-none"
          style={{
            ...
            // scale slightly to avoid blur fringe at edges
            transform: "scale(1.08)",
            filter: "blur(14px) brightness(0.22) saturate(0.75)",
          }}
        />
      )}

      {/* Dark vignette overlay — deepens the effect so the photo never competes */}
      {bg.url && (
        <div
          aria-hidden="true"
          className="fixed inset-0 pointer-events-none"
          style={{
            zIndex: -8,
            background:
              "radial-gradient(ellipse at center, rgba(13,26,38,0.55) 0%, rgba(13,26,38,0.85) 100%)",
          }}
        />
      )}

      {/* Starfield always on top of background */}
      <StarCanvas />
    </>
  );
}
