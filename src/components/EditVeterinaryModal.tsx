...
export default function EditVeterinaryModal({ isOpen, onClose, onSuccess, vet }: Props) {
    ...
    useEffect(() => {
        if (isOpen && vet) {
            ...
            setAddress({
                ...
                interiorNum: vet.interiorNum || "",
                neighborhood: vet.neighborhood || "",
                city: vet.city || "",
                state: vet.state || "",
                country: vet.country || "México",
                zipCode: vet.zipCode || "",
                latitude: vet.latitude ? String(vet.latitude) : "",
                longitude: vet.longitude ? String(vet.longitude) : "",
            });
        }
    }, [isOpen, vet]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`/api/veterinarias/${vet.id}`, {
                method: "PATCH",
