...
export default function RegisterClientModal({ isOpen, onClose, onSuccess }: RegisterClientModalProps) {
    ...
    const handleAddressChange = (updated: Partial<AddressValues>) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setStep(1);
                setServiceType(null);
                setFormData({ name: "", email: "", phone: "", petName: "", petSpecies: "Perro", petBreed: "" });
        ...
        <AnimatePresence>
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-bg-deep/80 backdrop-blur-sm">
                <motion.div
                    ...
                    className="glass-card w-full max-w-xl rounded-3xl overflow-hidden relative shadow-2xl border-white/10 max-h-[92vh] flex flex-col"
                >
                    {/* Header */}
                    ...
                    <div className="p-8 overflow-y-auto flex-1">
                        ...
                        {/* ── STEP 1: Client info + address ── */}
                        {step === 1 && (
                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">

                                {/* Personal data */}
                                <div className="flex items-center gap-3 text-brand-gold-500">
                                    <User size={20} />
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-slate-200 outline-none focus:border-brand-gold-500/50 transition-colors text-sm"
                        ...
                        {/* ── STEP 3: Pet info (immediate only) ── */}
                        {step === 3 && serviceType === "IMMEDIATE" && (
                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                                ...
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Nombre de la Mascota</label>
                                    <input
                                        type="text"
